...
public class GrantMJ extends AdminCommand
{
	...
	public void executeCommand(Player admin, String[] params)
	{
		if (params == null || params.length > 1) {
			PacketSendUtility.sendMessage(admin, "syntax //makemj [player]");
			return;
		}
		if ((params.length==0)||(params[0].trim().length()==0)) {
			if (admin.getTarget()==null) {
				PacketSendUtility.sendMessage(admin, "You haven't selected a player, use \"//makemj player\" instead");
				return;
			}
		}
		Player player = null;
		...
		if (player != null) {
			...
			PacketSendUtility.sendMessage(player, "[MJ] ("+admin.getName()+") make you MJ");
		} else {
