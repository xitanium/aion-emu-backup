...
public class AionPacketHandlerFactory
{
	...
	public AionPacketHandlerFactory(Injector injector)
	{
		...
		addPacket(new CM_SHOW_FRIENDLIST(Version.Chiness ? 0xE2 : 0x58), State.IN_GAME); // 1.5.x maybe not the right order
		addPacket(new CM_SHOW_BLOCKLIST(Version.Chiness ? 0xA4 : 0x12), State.IN_GAME);  // 0x12 or 0x58
		addPacket(new CM_SEARCH_STATUS(Version.Chiness ? 0xEC : 0x4A), State.IN_GAME); // 1.5.x unknown
		addPacket(new CM_RESTORE_CHARACTER(Version.Chiness ? 0xB5 : 0x03), State.AUTHED); 
		addPacket(new CM_QUIT(Version.Chiness ? 0x4F : 0xED), State.AUTHED, State.IN_GAME); // 1.5.x not sure
		...
		addPacket(new CM_FRIEND_STATUS(Version.Chiness ? 0xA6 : 0x14), State.IN_GAME); // 1.5.x not sure
