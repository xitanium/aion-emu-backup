...
public class SM_CREATE_CHARACTER extends PlayerInfo
{
	...
	protected void writeImpl(AionConnection con, ByteBuffer buf)
	{
		...
		if(responseCode == RESPONSE_OK)
		{
			...
			int uniqueId;
			if(player.getPlayerCommonData().getPlayerClass().getClassId() == 0)
			{// warrior
				uniqueId = inventory.putItemToDb(activePlayer, 100000094, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.MAIN_HAND);// sword for training
				uniqueId = inventory.putItemToDb(activePlayer, 110500003, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.ARMOR);// Chain armor for training
			...
			if(player.getPlayerCommonData().getPlayerClass().getClassId() == 3)
			{ // SCOUT
				uniqueId = inventory.putItemToDb(activePlayer, 100200112, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.MAIN_HAND);// Dagger for training
				uniqueId = inventory.putItemToDb(activePlayer, 110300015, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.ARMOR);// Leather Jerkin for training
			...
			if(player.getPlayerCommonData().getPlayerClass().getClassId() == 6)
			{// MAGE
				uniqueId = inventory.putItemToDb(activePlayer, 100600034, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.MAIN_HAND);// SpellBook for training
				uniqueId = inventory.putItemToDb(activePlayer, 110100009, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.ARMOR);// Cloth Tunic for Training
			...
			if(player.getPlayerCommonData().getPlayerClass().getClassId() == 9)
			{// PRIEST
				uniqueId = inventory.putItemToDb(activePlayer, 100100011, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.MAIN_HAND);// Mace for training
				uniqueId = inventory.putItemToDb(activePlayer, 110300292, 1);
				inventory.putIsEquipedToDb(uniqueId, 1, ItemSlot.ARMOR);// Leather armor for training
