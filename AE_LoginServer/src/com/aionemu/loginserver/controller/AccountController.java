	...
	public static synchronized void checkAuth(SessionKey key, GsConnection gsConnection)
	{
		...
		if (con != null)
		{
			if (con.getSessionKey().checkSessionKey(key))
			{
				 ...
				 */
				gsConnection.sendPacket(new SM_ACOUNT_AUTH_RESPONSE(key.accountId, true, acc.getName()));
			}
		}
		else
			gsConnection.sendPacket(new SM_ACOUNT_AUTH_RESPONSE(key.accountId, false, null));
	}
